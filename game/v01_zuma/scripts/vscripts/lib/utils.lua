...
if IsServer() then
	...
	function CDOTA_BaseNPC:ModifyMaxHealth(fChanged)
		if not self.fOriginalBaseHealth then
			self.fOriginalBaseHealth = self:GetMaxHealth()
		end
		...
		local fBonusHealth = AdditionMultiplicationPercentage(fAddHpPercent, fEnemyHpPercent) * self.fBaseHealth
		local fHealth = self.fBaseHealth + fBonusHealth
		local fCorrectHealth = Clamp(fHealth, 1, MAX_HEALTH)
	...
	function CEntities:FindAllByModifier(sModifierName)
		local t = {}
		local lastEnt = self:First()
		while IsValid(lastEnt) do
			if lastEnt.HasModifier and lastEnt:HasModifier(sModifierName) then
				table.insert(t, lastEnt)
			end
	...
	function CreateUnitByNameWithNewData(sUnitName, vLocation, bFindClearSpace, hNpcOwner, hUnitOwner, iTeamNumber, tOverrideData)
		...
		hUnit:SetOwner(hNpcOwner or hUnitOwner)
		return hUnit
	end
	...
	function CDOTA_BaseNPC:GetPreAttackSound()
		return (KeyValues:GetUnitData(self, "SoundSet") or "") .. ".PreAttack"
	end
	function CDOTA_BaseNPC:GetAttackSound()
		return (KeyValues:GetUnitData(self, "SoundSet") or "") .. ".Attack"
	end
	function CDOTA_BaseNPC:GetAttackImpactSound()
		return (KeyValues:GetUnitData(self, "SoundSet") or "") .. ".Attack.Impact"
	end
	function CDOTA_BaseNPC:GetProjectileImpactSound()
		return (KeyValues:GetUnitData(self, "SoundSet") or "") .. ".ProjectileImpact"
	end
	...
	local function Hook_ProjectileHid(self, hTarget, vLocation, ExtraData, tProjectileInfo, ...)
		...
		if nil == hTarget or ExtraData.typeProjectile == "Tracking" then
			RemoveHashtable(ExtraData.iProjectileInfoID)
		end
		if 'function' == type(self._OnProjectileHit_ExtraData) then
			return self._OnProjectileHit_ExtraData(self, hTarget, vLocation, tProjectileInfo.ExtraData, ...)
		end
	end
	local function Hook_CreateProjectile(typeProjectile, self, tInfo, ...)
		if tInfo then
			if not tInfo.ExtraData then
				tInfo.ExtraData = {}
			end
	...
	ProjectileManager.CreateTrackingProjectile = function(...)
		Hook_CreateProjectile("Tracking", ...)
	...
	ProjectileManager.CreateLinearProjectile = function(self, tInfo, ...)
		...
		Hook_CreateProjectile("Linear", self, tInfo, ...)
		return ProjectileManager._CreateLinearProjectile(self, tInfo, ...)
	end

	function CDOTA_BaseNPC_Creature:LevelUp(bPlayEffects, iLevels)
		if iLevels == nil then
			iLevels = 1
		end
		...
		local fManaPercent = self:GetMana() / self:GetMaxMana()
		local fHealthPercent = self:GetHealth() / self:GetMaxHealth()
		self._bIsLevelUping = true
		self:CreatureLevelUp(iLevels)
		self._bIsLevelUping = false
		...
		for i, tData in pairs(hAbilities) do
			local hAbility = self:GetAbilityByIndex(i)
			if hAbility then
				local bNonLevel = tData.iLevel == 0
				hAbility:SetLevel(tData.iLevel)
				if bNonLevel then
					tData.bAutoCastState = false
					tData.bToggleState = false
				end
...
function CDOTA_Buff:GetPlayerID()
	return self:GetParent():GetPlayerOwnerID()
end
...
function CalculateDistance(ent1, ent2, b3D)
	...
	if ent1.GetAbsOrigin then
		if not IsValid(ent1) then return 0 end
		pos1 = ent1:GetAbsOrigin()
	end
	if ent2.GetAbsOrigin then
		if not IsValid(ent2) then return 0 end
...
function CalculateDirection(ent1, ent2)
	...
	local pos2 = ent2
	if ent1.GetAbsOrigin then
		if not IsValid(ent1) then return vec3_invalid end
		pos1 = ent1:GetAbsOrigin()
	end
	if ent2.GetAbsOrigin then
		if not IsValid(ent2) then return vec3_invalid end
		pos2 = ent2:GetAbsOrigin()
	end
...
function super(self, sFunctionName, ...)
	local base = getbase(getclass(self))
	local func = rawget(base, sFunctionName)
